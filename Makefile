...
define build_for_device
	SDK=$(DEVELOPER)/Platforms/$(2).platform/Developer/SDKs/$(2)$(SDK_VERSION).sdk; \
	export CC=$(TOOLCHAIN)/usr/bin/clang; \
	export CFLAGS="-arch $(1) -isysroot $$SDK -miphoneos-version-min=$(IPHONEOS_DEPLOYMENT_TARGET) -fembed-bitcode"; \
...
define build_for_simulator
	SDK=$(DEVELOPER)/Platforms/$(2).platform/Developer/SDKs/$(2)$(SDK_VERSION).sdk; \
	export CC=$(TOOLCHAIN)/usr/bin/clang; \
	export CFLAGS="-arch $(1) -isysroot $$SDK -mios-simulator-version-min=$(IPHONEOS_DEPLOYMENT_TARGET) -fembed-bitcode -target $(1)-apple-ios-simulator"; \
...
_create_xcframework:
	...
	# Create XCFramework from fat library for the simulator and device library
